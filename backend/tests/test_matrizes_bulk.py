...
from app.modules.fiscal.api.matrizes_bulk import exportar_csv, importar_csv
...
async def test_export_vazia_devolve_so_o_cabecalho_template(sessao):
    csv = await exportar_csv(sessao, "mva")
...
async def test_import_valida_upserta_e_relata_erros(sessao):
    ...
    r = await importar_csv(sessao, "mva", conteudo)
